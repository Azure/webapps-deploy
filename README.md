...
* Authenticate using [Azure Web App Publish Profile](https://github.com/projectkudu/kudu/wiki/Deployment-credentials#site-credentials-aka-publish-profile-credentials) or using [Azure Login](https://github.com/Azure/login). Publish profile is supported for Azure Web Apps(both Windows and Linux) and Azure Web App Containers(Linux only). Also, publish profile scheme does not support multi-container scenario.
