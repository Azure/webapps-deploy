...
Build, unit, and Azure integration tests run through `azure-pipelines.yml`. Pull requests run cross-platform validation without Azure credentials. Deployment tests run only for trusted branch builds and use an Azure DevOps service connection.

Before enabling the pipeline, configure the existing publish profiles, `AZURE_WEBAPP_PUBLISH_PROFILE`, `WEBDEPLOY_TEST_ACR_USERNAME`, and `WEBDEPLOY_TEST_ACR_PASSWORD` as secret Azure Pipelines variables.
