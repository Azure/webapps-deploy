...
class Kudu {
    ...
    oneDeploy(webPackage, queryParameters, trackStatus, appService) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (response.statusCode == 202 || response.statusCode == 200) {
                    if (!!trackStatus && trackStatus == "true") {
                        core.debug(`polling the status of deployment`);
                        let deploymentID = yield this.getLatestDeploymentID();
                        if (!deploymentID) {
                            var deploymentStatusUrl = this._appService.buildDeploymentStatusUrl(deploymentID);
                            core.debug(`Deployment status url: ${deploymentStatusUrl}`);
                            var deploymentStatusResponse = yield this.trackDeploymentStatus(deploymentStatusUrl, deploymentID, timeout);
                        }
                        else {
                            core.debug(`Failed to enable tracking runtime status for this deployment. Resuming without tracking status. \n`);
                            let pollableURL = response.headers.location;
                            if (!!pollableURL) {
    ...
    trackDeploymentStatus(deploymentStatusUrl, deploymentID, timeout) {
        return __awaiter(this, void 0, void 0, function* () {
            var startTime = new Date().getTime();
            var timeElapsed = 0;
            var timeOutInMilliSeconds = timeout * 1000;
            var httpRequest = {
                method: 'GET',
                uri: this._client.getRequestUri(deploymentStatusUrl)
            };
            while (timeElapsed < timeOutInMilliSeconds) {
                try {
                    var response = yield this._client.beginRequest(httpRequest);
                    core.debug(`Deployment Status API response data: ${JSON.stringify(response)}`);
                    var responseBody = JSON.parse(response.body);
                    var deploymentProperties = responseBody.properties;
                    var deploymentStatus = deploymentProperties.status;
                    timeElapsed = new Date().getTime() - startTime;
                    if (deploymentStatus in DEPLOYMENT_STATUS_MAP) {
                        core.debug(`Deployment status: ${DEPLOYMENT_STATUS_MAP[deploymentStatus]}.`);
                        if (deploymentStatus === DEPLOYMENT_STATUS.RuntimeSuccessful) {
                            core.debug(`Deployment ${deploymentStatus} in ${timeElapsed / 1000} seconds`);
                            return responseBody;
                        }
                        else if (deploymentStatus === DEPLOYMENT_STATUS.RuntimeStarting) {
                            core.debug(`InProgress Instances: ${deploymentProperties.numberOfInstancesInProgress}.\n Successful instances: ${deploymentProperties.numberOfInstancesSuccessful}`);
                        }
                        else if (deploymentStatus === DEPLOYMENT_STATUS.RuntimeFailed) {
                            let totalInstances = deploymentProperties.numberOfInstancesInProgress + deploymentProperties.numberOfInstancesSuccessful + deploymentProperties.numberOfInstancesFailed;
                            if (deploymentProperties.numberOfInstancesSuccessful > 0) {
                                core.debug(`Site started with errors.${deploymentProperties.numberOfInstancesFailed}/${totalInstances} instances failed to start successfully.`);
                            }
                            else {
                                core.debug(`Deployment failed because the site failed to start within 10 minutes.`);
                                core.debug(`InProgress Instances: ${deploymentProperties.numberOfInstancesInProgress}.\n Successful instances: ${deploymentProperties.numberOfInstancesSuccessful}.\n Failed instances: ${deploymentProperties.numberOfInstancesFailed}`);
                            }
                            let errors = deploymentProperties.errors;
                    ...
                    throw Error(`Deployment status endpoint ${deploymentStatusUrl} returned error .\n` + this._getFormattedError(error));
                }
                this._sleep(15);
            }
        });
    }
    // Get the latest deployment ID from the list of deployments
    getLatestDeploymentID() {
        return __awaiter(this, void 0, void 0, function* () {
            var totalTrials = 30;
            var numTrials = 0;
            var responseBody = null;
            var latestDeploymentUrl = `/api/deployments/latest`;
            while (numTrials < totalTrials) {
                yield this._sleep(2);
                let httpRequest = {
                    method: 'GET',
                    uri: this._client.getRequestUri(latestDeploymentUrl)
                };
                try {
                    let response = yield this._client.beginRequest(httpRequest);
                    core.debug(`Latest Deployment ID response data: ${JSON.stringify(response)}`);
                    try {
                        responseBody = JSON.parse(response.body);
                    }
                    catch (error) {
                        throw Error(`Deployment status endpoint ${latestDeploymentUrl} returned malformed data. \n` + this._getFormattedError(error));
                    }
                    finally {
                        numTrials++;
                    }
                    if (!!responseBody.id && !responseBody.id.startsWith("temp")) {
                        return responseBody.id;
                    }
                }
                catch (error) {
                    throw Error("Failed to get the latest deployment ID.\n" + this._getFormattedError(error));
                }
            }
        });
    }
