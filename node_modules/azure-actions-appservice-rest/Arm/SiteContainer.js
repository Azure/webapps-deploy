...
(function (AUTH_TYPE) {
    ...
    AUTH_TYPE["SYSTEM_ASSIGNED"] = "SystemAssigned";
...
class VolumeMount {
    constructor(containerMountPath, data, readOnly, volumeSubPath) {
    ...
    static fromJson(item) {
        return new VolumeMount(item.containerMountPath, item.data, item.readOnly, item.volumeSubPath);
    }
...
class SiteContainer {
    constructor(name, image, targetPort, isMain, startupCommand, authType, userName, passwordSecret, userManagedIdentityClientId, type, environmentVariables, volumeMounts) {
        this.name = name;
        this.image = image;
