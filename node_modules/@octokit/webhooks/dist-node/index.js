...
const WEBHOOK_HEADERS$1 = ["x-github-event", "x-hub-signature-256", "x-github-delivery"]; // https://docs.github.com/en/developers/webhooks-and-events/webhook-events-and-payloads#delivery-headers

function getMissingHeaders$1(request) {
  return WEBHOOK_HEADERS$1.filter(header => !(header in request.headers));
}

// @ts-ignore to address #245
function getPayload$1(request) {
  // If request.body already exists we can stop here
  // See https://github.com/octokit/webhooks.js/pull/23
  if (request.body) return Promise.resolve(request.body);
  return new Promise((resolve, reject) => {
...
async function middleware$1(webhooks, options, request, response, next) {
  ...
  const missingHeaders = getMissingHeaders$1(request).join(", ");
  ...
  try {
    const payload = await getPayload$1(request);
