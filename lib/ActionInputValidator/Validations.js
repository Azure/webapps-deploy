...
function validateContainerInputs() {
    ...
    // this error occurs when the config file doesn't exist and no images are given.
    ...
    // this error occurs when multiple images are there and the config file is absent.
    else if (actionParams.images.split("\n").length > 1) {
        throw new Error("Multiple images indicate multi-container deployment type, but Docker-compose file is absent.SG");
    }
}
